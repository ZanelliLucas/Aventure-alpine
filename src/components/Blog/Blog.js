...
const Blog = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        ...
        setError('Erreur lors de la récupération des articles de blog');
        setLoading(false);
  ...
  useEffect(() => {
    const applyFilters = () => {
      ...
      // NOTE: Normalement, vous auriez une propriété category dans vos articles
      // Cet exemple suppose qu'elle existe, mais votre API actuelle ne l'a peut-être pas encore
      if (categoryFilter) {
  ...
  // Dans une vraie application, ces données viendraient de votre API
  const categories = [
    "Randonnée",
    "Escalade",
    "Ski alpin",
  ...
  return (
    <div className="blog-page">
      <Container>
        <h1 className="page-title">Blog Aventures Alpines</h1>
        
        {/* Bannière d'introduction */}
        <div className="intro-banner">
          <div className="intro-content">
            <h2>Découvrez nos articles et conseils</h2>
            <p>
              Restez informés des dernières tendances, récits d'aventures et conseils d'experts
              pour tirer le meilleur parti de vos expériences en montagne.
            </p>
          </div>
        </div>
          ...
          <Card>
            <Card.Body>
              <h3 className="filter-title">
                <FaFilter className="filter-icon" /> Rechercher des articles
              </h3>
              <Row>
                <Col md={5}>
